...
proc customRound(value: float, precision: int32): float =
  ...
  if abs(fraction) < 0.5: result = floor(scaled) / factor
  elif abs(fraction) > 0.5: result = ceil(scaled) / factor
  else: result = (round(scaled / 2.0) * 2.0) / factor
...
proc generateJsonData*(n: int64): JsonData =
  ...
  for i in 0..<n:
    result.coordinates[i] = Coordinate(
      x: customRound(nextFloat(), 8),
      y: customRound(nextFloat(), 8),
      z: customRound(nextFloat(), 8),
      name: &"{nextFloat():.7f} {nextInt(10000)}",
      opts: opts
    )
